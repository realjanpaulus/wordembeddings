...
def main():
    ...
    if args.save_confusion_matrices:
        ...
        cm_df.to_csv(
            f"../results/cm_bert_{args.splitnumber}_lr{args.learning_rate}.csv"
        )
    ...
    logging.info(
        "Training took {:} (h:mm:ss)".format(utils.format_time(time.time() - total_t0))
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Indicates the pretrained model.",
    )
    parser.add_argument(
        "--patience",
        "-p",
