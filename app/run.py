...
def main():
    ...
    logging.basicConfig(level=logging.DEBUG, filename=f"../logs/run.log", filemode="w")
    ...
    cartesian_inputs = list(
        product(batch_sizes, learning_rates, embeddings, max_features, split_numbers)
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        prog="run", description="Runs cnn script with multiple arguments."
    )
    parser.add_argument(
        "--embedding_type",
        "-et",
        type=str,
        default="fasttext",
        help="Indicates embedding type. Possible values: 'fasttext', 'glove'.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "-mp",
        action="store_true",
        help="Uses more hyperparameter for tuning.",
    )
    parser.add_argument(
